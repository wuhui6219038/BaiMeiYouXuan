...
public class Ruler extends View {
    ...
    private int oldY;
    //基本刻度长度
    private float ordinaryLineHeigetValue = 15;
    //每种刻度之间的差值
    private float offsetLineHeight = 5;
    ...
    public Ruler(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        interval = array.getDimensionPixelSize(R.styleable.Ruler_interval, SizeUtils.dp2px(intervalsBetweenValues));
        ...
        valuesTextSize = array.getDimensionPixelSize(R.styleable.Ruler_valuesTextSize, SizeUtils.sp2px(16));
        ...
        linesWidth = array.getDimensionPixelSize(R.styleable.Ruler_linesWidth, SizeUtils.dp2px(2));
        centerlineWidth = array.getDimensionPixelSize(R.styleable.Ruler_centerLineWidth, SizeUtils.dp2px(3));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (orientation == VERTICAL) {
            ...
            drawHorzontal(canvas);
        }

    }

    private void drawHorzontal(Canvas canvas) {

        //普通刻度线宽度
        int ordinaryLineHeiget = SizeUtils.dp2px(ordinaryLineHeigetValue);
        int mSubHeight = SizeUtils.dp2px(ordinaryLineHeigetValue + offsetLineHeight);
        //整数刻度线
        int mHeiget = SizeUtils.dp2px(ordinaryLineHeigetValue + offsetLineHeight*2);
        int chooseLineHeiget = SizeUtils.dp2px(ordinaryLineHeigetValue + offsetLineHeight*3);
        //刻度值宽度
        int textRealHeight = chooseLineHeiget + textHeight;
        //画中间的指针
        ...
        while (true) {
            if (position % (intervalsBetweenValues / 2) == 0) {
                ...
                canvas.drawLine(width, 0, width, ordinaryLineHeiget, paint);
            }
            position--;
        ...
        canvas.drawLine(getWidth() / 2, 0, getWidth() / 2, chooseLineHeiget, centerPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (listener != null) {
                    //通过一个接口将数据暴露出去
                    listener.onValueChange(currentPosition * valuesInterval / (double) intervalsBetweenValues);
                }
                break;
    ...
    public void setValue(double value) {
        Log.e(TAG, "setValue: " + value / valuesInterval * intervalsBetweenValues);
        currentPosition = (int) (value / valuesInterval * intervalsBetweenValues);
        invalidate();
    }
